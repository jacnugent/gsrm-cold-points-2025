...
def parse_args():
    ...
    parser.add_argument("-c", "--conv_thresh", help="Convective threshold for qi (kg/kg) (will be converted to a float)")
...
def bin_var_by_tb(ds_all, tb_dar, varname, bins=TB_BINS, save_dicts=False, pickle_dir=PICKLE_DIR,
    ...
    """
    if varname == "qi" or varname == "qi_conv":
...
def bin_var_by_diffs(ds_all, diffs, varname, bins=DIFF_BINS, save_dicts=False, pickle_dir=PICKLE_DIR,
                      month=None, year=None, region=None, qi_min=QI_MIN, ref_densities=REF_DENSITIES, iwc_min=IWC_MIN, iflag=None, conv_thresh=CONV_THRESH, racp_thresh=RACP_THRESH):
    """ 
    ...
    Returns list of dictionaries and the bins: [bin_means_dict, bin_counts_dict, bins].
    """
    if varname == "qi" or varname == "qi_conv" or varname == "qi_racp":
    ...
    for key in var_levs_dict.keys():
        ...
        elif varname == "qi_conv":
            var_lev = var_levs_dict[key].where(var_levs_dict[key] > conv_thresh)
        elif varname == "qi_racp":
            var_lev = var_levs_dict[key].where(var_levs_dict[key] > racp_thresh)
        else:
            var_lev = var_levs_dict[key]
    ...
    if save_dicts:
        ...
        if iflag is not None:
            mean_name = pickle_dir + "{v}_iflag{f}_30min_bin_means_{m}{y}_{r}.pickle".format(v=varname, f=iflag, m=month, y=year, r=region)
            count_name = pickle_dir + "{v}_iflag{f}_30min_bin_counts_{m}{y}_{r}.pickle".format(v=varname, f=iflag, m=month, y=year, r=region)
        if varname == "qi_conv" and iflag is None:
            # add convective threshold to the file name
            mean_name = pickle_dir + "{v}_{t}_30min_bin_means_{m}{y}_{r}.pickle".format(v=varname, m=month, y=year, r=region, t=str(conv_thresh))
            count_name = pickle_dir + "{v}_{t}_30min_bin_counts_{m}{y}_{r}.pickle".format(v=varname, m=month, y=year, r=region, t=str(conv_thresh)) 
        else:
            mean_name = pickle_dir + "{v}_30min_bin_means_{m}{y}_{r}.pickle".format(v=varname, m=month, y=year, r=region)
            count_name = pickle_dir + "{v}_30min_bin_counts_{m}{y}_{r}.pickle".format(v=varname, m=month, y=year, r=region)
        with open(mean_name, 'wb') as handle:
...
def plot_diffs_hist(diffs, month, region, year, bins=DIFF_BINS, dar_times_only=False, fsize=13, 
                    tsize=14, figsize=(7, 4), ylim=(0, 0.035),
                    gridlines=True, annotate_total=True, save=False, ylog=False,
                    save_dir=None, dar_file_path=FILE_PATH, dar_list_ds=None, return_hist=False):
    """ 
...
def plot_binned_by_tb(bin_dict, tb_dar, statistic, month, region, year, bins=TB_BINS,
                      stat_lims=None, fsize=26, tsize=30, figsize=[13, 6],
                       colormap="Spectral", cbar_ext="max", lxognorm=True, save=False,
...
def main():
    ...
    conv_thresh = float(args.conv_thresh)
    ...
    if bin_by_diffs:
        output_qi_conv = bin_var_by_diffs(ds_all, diffs_dar, "qi_conv", save_dicts=save_dicts, 
                                     pickle_dir=save_pickle_path,
                                   month=months, year=year, region=region,
                                         conv_thresh=conv_thresh)
        # output_qi_racp = bin_var_by_diffs(ds_all, diffs_dar, "qi_racp", save_dicts=save_dicts, 
        #                              pickle_dir=save_pickle_path,
        #                            month=months, year=year, region=region)
